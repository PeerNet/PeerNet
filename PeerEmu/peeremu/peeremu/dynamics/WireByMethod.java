...
public WireByMethod(String prefix)
{
	...
	try
	{
		...
		Class.forName("peeremu.graph.GraphFactory"));
	...
	for(int i=1; i<args.length; ++i)
	{
		...
		else
		{
			...
			"report this problem to the peeremu team");
		}
	}
}
...
private static Method getMethod(Class cl, String methodName)
	...
	// it is a setter.
	final Class graphClass = Class.forName("peeremu.graph.Graph");
	...
	if( pars.length < 1 || !pars[0].isAssignableFrom(graphClass) )
		...
		" it has to have peeremu.graph.Graph as first argument type");
