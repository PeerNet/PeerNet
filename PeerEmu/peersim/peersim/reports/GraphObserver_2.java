...
protected GraphObserver_2(String name) {
	...
	if (undir && reverse)
	{
		throw new IllegalParameterException(
			name + "." + PAR_UNDIR + ", "+ name + "." + PAR_REVERSE,
...
protected void updateGraph() {
	...
	if( CommonState.getTime() != GraphObserver_2.time ||
	    (CDState.isCD() && (CDState.getCycleT() != GraphObserver_2.ctime)) ||
	    ...
	    pid != GraphObserver_2.lastpid )
	{
		...
		if( CDState.isCD() ) GraphObserver_2.ctime = CDState.getCycleT();
		...
		{
			if( fast )
				GraphObserver_2.undirg =
				new FastUndirGraph(GraphObserver_2.dirg);
			else
				GraphObserver_2.undirg =
				new ConstUndirGraph(GraphObserver_2.dirg);
		}
		if( GraphObserver_2.needReverse )
		{
			GraphObserver_2.reverseg =
			new ReverseGraph(GraphObserver_2.dirg);
		}
	}
	
	if( undir ) g = GraphObserver_2.undirg;
	else if( reverse ) g = GraphObserver_2.reverseg;
	else g = GraphObserver_2.dirg;
}
