 ...
 * parameter {@value peeremu.core.Scheduler#PAR_STEP} of {@link Scheduler}).
...
public class CDScheduler implements Control, NodeInitializer
{
   ...
   * scheduler and the parameters of the protocol (cycle length). Protocols are
   * scheduled independently of each other.
   * 
   * @config
   */
   ...
   * execution is detemined by {@link #firstDelay}. The implementation calls
   ...
   * {@value peeremu.core.Scheduler#PAR_FROM} of the protocol being scheduled.
   ...
   * use parameter {@value peeremu.core.Scheduler#PAR_FROM} in protocols, but to
  ...
  {
    /*
     * XXX If "from" is not the current time and this is used as a control (not
     * node initializer) then we dump _lots_ of events in the queue that are
     * just stored there until "from" comes. This reduces performance, and
     * should be fixed. When fixed, the final comment can be removed from the
     * docs.
     */
    ...
    for (int i = 0; i<pid.length; ++i)
    {
      ...
      /*
       * If step <= 0, do not schedule the execution of this protocol. This is a
       * logical way to turn off specific protocols.
       */
      if (sch[pid[i]].step<=0)
        continue;
  ...
  /**
   * Returns the time (through giving the delay from the current time) when this
   * even is first executed. If {@value #PAR_RNDSTART} is not set, it returns
   * zero, otherwise a random value between 0, inclusive, and cyclelength,
   * exclusive.
   * 
   * @param cyclelength The cycle length of the cycle based protocol for which
