...
{
  /**
   * Constant that can be used as a value of simulation phase. It means that the
  ...
  /**
   * Constant that can be used as a value of simulation phase. It means that the
   * simulation phase information has not been (unknown).
   * 
   * @see #getPhase
   */
...
//  private static int toshift = -1;
   ...
   * {@link #setEndTime} by the simulation engine. It uses the endtime for the
   * optimal mapping to get the maximal precision. In particular, in the cycle
...
//      toshift = 0;
  ...
  /**
   * Returns the current protocol identifier. In other words, control is held by
   * the indicated protocol on node {@link #getNode}.
   */
  public static int getPid() // TODO: Remove global access. Not good for
                             // multithreaded case.
  {
    return pid;
  }
