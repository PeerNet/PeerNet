...
/**
 * This {@link Control} invokes a specified method on a protocol. The method is
 * defined by config parameter {@value #PAR_METHOD} and the protocol is by
 * {@value #PAR_PROT}. The method must not have any parameters and must return
 * void. If no protocol is specified, then the method will be invoked on all
...
public class MethodInvoker implements Control, NodeInitializer
{
  ...
  // --------------------------------------------------------------------------
  /**
   * The protocol to operate on. If not defined, the given method will be
   * invoked on all protocols that define it. In all cases, at least one
   * protocol has to define it.
   * 
   * @config
   */
  ...
  public MethodInvoker(String prefix)
  {
    ...
    if (!Configuration.contains(prefix+"."+PAR_PROT))
    {
      ...
      for (int i = 0; i<Network.prototype.protocolSize(); ++i)
      {
        ...
        try
        {
          m = MethodInvoker.getMethod(Network.prototype.getProtocol(i)
              .getClass(), methodName);
        }
      ...
      if (pids.isEmpty())
      {
        throw new IllegalParameterException(prefix+"."+PAR_METHOD,
            "No protocols found that implement 'void "+methodName+"()'");
      }
    ...
    else
    {
      ...
      try
      {
        ...
        method[0] = MethodInvoker.getMethod(
            Network.prototype.getProtocol(pid[0]).getClass(), methodName);
      }
      catch (NoSuchMethodException e)
      {
      ...
      throws NoSuchMethodException
  {
    ...
    if (list.size()==0)
    {
      throw new NoSuchMethodException("Method "+methodName+" in class "+
          cl.getName());
    }
    return list.get(0);
  }
