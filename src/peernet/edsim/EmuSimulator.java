...
public class EmuSimulator extends EDSimulator
{
  ...
  private static void scheduleControls()
  {
    ...
    {
      ControlEvent event = new ControlEvent(controls[i], ctrlSchedules[i],
          order++);
      if (order>((1<<rbits)-1))
      ...
      else
      {
        ...
        catch (ClassCastException e)
        {
          throw new IllegalArgumentException("Protocol "+
              Configuration.lookupPid(pid)+" does not implement EDProtocol; "+
              ev.event.getClass());
        }
      }
    }
  ...
  public static void add(long delay, Object event, Node node, int pid)
  {
    if (delay<0)
      throw new IllegalArgumentException("Protocol "+node.getProtocol(pid)+
          " is trying to add event "+event+" with a negative delay: "+delay);
    if (pid>Byte.MAX_VALUE)
      throw new IllegalArgumentException(
          "This version does not support more than "+Byte.MAX_VALUE+
              " protocols");
