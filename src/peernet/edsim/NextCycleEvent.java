...
public class NextCycleEvent implements Cloneable
{
  ...
  // =====================================================================
  /**
   * Reads configuration to initialize the object. Extending classes should have
   * a constructor with the same signature, often as simple as
   * <code>super(n)</code>.
   */
   ...
   * using the delay returned by {@link #nextDelay}. If the next execution time
   * as defined by the delay is outside of the valid times as defined by
   * {@link CDScheduler#sch}, then the next event is not scheduled. Note that
   * this means that this protocol will no longer be scheduled because the next
   * event after the next event is scheduled by the next event.
   */
  ...
  // --------------------------------------------------------------------
  /**
   * Calculates the delay until the next execution of the protocol. This default
   * implementation returns a constant delay equal to the step parameter (cycle
   * length in this case) of the schedule of this event (as set in the config
   * file).
   */
