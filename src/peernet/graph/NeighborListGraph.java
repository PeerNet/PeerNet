...
/**
 * Implements a graph which uses the neighbor list representation. No multiple
 * edges are allowed. The implementation also supports the growing of the graph.
 * This is very useful when the number of nodes is not known in advance or when
 * we construct a graph reading a file.
 */
public class NeighborListGraph implements Graph, java.io.Serializable
{
  // =================== private fields ============================
  // ===============================================================
  /** Contains the objects associated with the node indices. */
  private final ArrayList<Object> nodes;
  /**
   * Contains the indices of the nodes. The vector "nodes" contains this
   * information implicitly but this way we can find indices in log time at the
   * cost of memory (node that the edge lists typically use much more memory
   * than this anyway). Note that the nodes vector is still necessary to provide
   * constant access to nodes based on indices.
   */
  private final HashMap<Object, Integer> nodeindex;
  /**
   * Contains sets of node indices. If "nodes" is not null, indices are defined
   * by "nodes", otherwise they correspond to 0,1,...
   */
   ...
   * constructed this way, it is not possible to associate objects to nodes, nor
   * it is possible to grow the graph using {@link #addNode}.
   * 
   * @param directed if true the graph will be directed
   */
   ...
   * Returns the index of the node. If the graph was constructed to have a
   * specific size, it is not possible to add nodes and therefore calling this
   * method will throw an exception.
   * 
   * @throws NullPointerException if the size was specified at construction
   *           time.
   */
   ...
   * associated with the node, otherwise null
   */
