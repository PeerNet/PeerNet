...
public class IncrementalStats
{
  ...
  /**
   * Resets the statistics to reflect the zero elements set. Min and max are set
   * to positive and negative infinity, respectively.
   */
  public void reset()
  {
    countmin = 0;
  ...
  /**
   * The empirical variance of the data items. Guaranteed to be larger or equal
   * to 0.0. If due to rounding errors the value becomes negative, it returns
   * 0.0.
   */
  public double getVar()
  {
    // XXX note that we have very little possibility to increase numeric
    // stability if this class is "greedy", i.e., if it has no memory.
    ...
    double var = (((double) n)/(n-1))*(sqrsum/n-getAverage()*getAverage());
   ...
   * this order. Minimum, maximum, number of items, average, variance, number of
   * minimal items, number of maximal items.
   */
