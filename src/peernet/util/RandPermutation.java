...
public class RandPermutation implements IndexIterator
{
   ...
   * permutation was of the same length, it is more efficient. Note that after
   * calling this the object is reset, so {@link #next} can be called k times,
   * even if {@link #get} was called an arbitrary number of times. Note however
   * that mixing {@link #get} and {@link #next} results in incorrect behavior
   * for {@link #get} (but {@link #next} works fine). The idea is to use this
   * method only in connection with {@link #get}.
   */
...
//  }
   ...
   * If {@link #next} is called after {@link #setPermutation} and before this
   * method, then the behavior of this method is unspecified.
   */
   ...
   * method {@link #next}. Calls to {@link #get} return undefined values, so
   * {@link #next} must be used. If the previous permutation was of the same
   * length, it is more efficient.
   */
